...
var matchPath = require('react-router').matchPath;

var ALL_INITIALIZERS = [];
var READY_INITIALIZERS = [];
var ROUTE_INITIALIZERS = {};
...
function createLoadableComponent(loadFn, options) {
  ...
  if (opts.route) {
    if (typeof opts.route === 'string') {
      ...
      var matchRoutes = opts.route;
      while (matchRoutes.length) {
        var route = matchRoutes.pop();
...
Loadable.preloadRoute = function (route) {
  var initializers = [];
  for (var path in ROUTE_INITIALIZERS) {
    var match = matchPath(route, { path: path, exact: true });
    if (match != null) {
